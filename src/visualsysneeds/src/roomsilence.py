...
class RoomSilence:
  ...
  def __init__(self):
    ...
    self.pub = rospy.Publisher('/opencog/roomsilence', String, queue_size=30)
    self.bridge = CvBridge()
    self.image_sub = rospy.Subscriber("/usb_cam_node/image_raw",Image, self.callback)

  def roomsilence(self, count, currentState):
  ...
  def callback(self,data):
    try:
        ...
        state = 0
        if activeSub >= 1:
            state = 1*0.75 # boosting the impact of moves as the number of subjects increase
        else:
            state= -1
        Data = str(activeSub)
